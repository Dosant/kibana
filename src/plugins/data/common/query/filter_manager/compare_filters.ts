...
const mapFilter = (
  ...
  const cleaned: Partial<Filter & FilterMeta> = omit(filter, excludedAttributes);
  ...
  if (comparators.state)
    cleaned.$state = { store: filter.$state?.store ?? FilterStateStore.APP_STATE }; // is state is missing use FilterStateStore.APP_STATE as default
